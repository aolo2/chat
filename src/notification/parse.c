...
static bool
p_startswith_str(struct bc_str str, struct bc_str prefix)
{
...
{
    struct bc_str body = *input;
    
    if (input->length < 16) {
        return(false);
    }
    
    s32 endpoint_length = 0;
    s32 auth_length = 0;
    s32 p256dh_length = 0;
    
    memcpy(&endpoint_length, input->data + 4, 4);
    memcpy(&auth_length, input->data + 8, 4);
    memcpy(&p256dh_length, input->data + 12, 4);
    
    if (input->length != endpoint_length + auth_length + p256dh_length + 16) {
        log_error("%d + %d + %d + 16 != %d\n", endpoint_length, auth_length, p256dh_length, input->length);
        return(false);
    }
...
parse_http_request(struct bc_str input)
{
    ...
    result.raw_length = input.length;
